...
Modelo: Vision (Variante otimizada para visão geral)
"""
...
class Config:
    """Configurações de treinamento - Vision Model"""
    ...
    # Model - VISION usa configuração diferente para otimização
    IMAGE_SIZE = 640
    ...
    WORKSPACE_DIR = BASE_DIR / 'workspace_vision'  # Diferente do Detector
    ...
    # Learning Rate - VISION usa learning rate mais agressivo
    LEARNING_RATE_BASE = 0.05  # Slightly higher than Detector
    WARMUP_LEARNING_RATE = 0.016667
    ...
    # Vision-specific: foco em long-range detection
    MIN_DETECTION_CONFIDENCE = 0.4  # Lower threshold for distant objects
    NMS_IOU_THRESHOLD = 0.5  # Non-max suppression threshold
...
def setup_directories():
    ...
    print("📁 Criando estrutura de diretórios (Vision)...")
...
def download_dataset():
    ...
    print(f"\n📥 Baixando dataset do Roboflow (Vision)...")
...
# ============================================================================
# CONFIGURAÇÃO DO PIPELINE - VISION OPTIMIZED
# ============================================================================

def create_pipeline_config(labelmap_path):
    """Cria arquivo de configuração personalizado para treinamento - VISION"""
    print("\n⚙️  Criando pipeline de configuração (Vision)...")
    
    # Paths
    base_config_path = Config.PRETRAINED_DIR / Config.CONFIG_FILE
    output_config_path = Config.WORKSPACE_DIR / 'pipeline_vision.config'
    ...
    num_train_images = 1000  # Placeholder
    ...
    # Modificações - VISION-specific optimizations
    replacements = {
    ...
    # VISION-specific: ajusta score threshold e IOU threshold
    config_text = re.sub(
    ...
    print(f"  Vision-specific: score_threshold={Config.MIN_DETECTION_CONFIDENCE}, iou_threshold={Config.NMS_IOU_THRESHOLD}")
...
def train_model(pipeline_config_path):
    """Executa treinamento do modelo Vision"""
    print("\n🚀 Iniciando treinamento (VISION Model)...")
    print(f"  Epochs: {Config.NUM_EPOCHS}")
    print(f"  Batch size: {Config.BATCH_SIZE}")
    print(f"  Total steps: {Config.TOTAL_STEPS}")
    print(f"  Image size: {Config.IMAGE_SIZE}x{Config.IMAGE_SIZE}")
    print(f"  Model variant: VISION (optimized for general vision)")
    ...
    print("\n" + "="*80)
    print("Comando de treinamento (Vision):")
    print(cmd)
    print("="*80 + "\n")
...
def apply_qat_to_checkpoint():
    ...
    print("\n🔧 Aplicando Quantization Aware Training (QAT) - Vision Model...")
    print("  Esta etapa refina o modelo para quantização INT8...")
    print("  Vision model: foco em robustez para diferentes condições de iluminação")
...
def export_to_saved_model(pipeline_config_path):
    ...
    print("\n📦 Exportando para SavedModel (Vision)...")
...
def convert_to_tflite():
    """Converte SavedModel para TFLite com quantização INT8 - Vision optimized"""
    print("\n🔄 Convertendo para TFLite com quantização INT8 (Vision)...")
    ...
    if Config.QUANTIZATION_TYPE == 'int8':
        print("  Aplicando quantização INT8 completa (Vision)...")
    ...
    # Salva
    tflite_filename = f'vision_mobilenet_v2_{Config.IMAGE_SIZE}_{Config.QUANTIZATION_TYPE}.tflite'
    ...
    print(f"  Modelo: VISION (optimized for general vision tasks)")
...
def test_tflite_model(tflite_path):
    """Testa modelo TFLite Vision com imagens de exemplo"""
    print(f"\n🧪 Testando modelo TFLite Vision...")
    ...
    print(f"\n  Testando Vision model com {len(test_images)} imagens...")
    ...
    for img_path in test_images:
        ...
        print(f"    {img_path.name}: {detections} detecções (threshold={Config.MIN_DETECTION_CONFIDENCE})")
...
def main():
    ...
    # Sumário final
    print("\n" + "="*80)
    print("✅ TREINAMENTO CONCLUÍDO COM SUCESSO! (VISION MODEL)")
    print("="*80)
    print(f"\n📁 Arquivos gerados:")
    print(f"  - Checkpoint: {Config.TRAINING_DIR}")
    print(f"  - TFLite Model: {tflite_path}")
    print(f"  - Labelmap: {Config.TFLITE_DIR / 'labelmap.txt'}")
    print("\n💡 Diferenças Vision vs Detector:")
    print("  - Vision: Otimizado para visão geral e objetos distantes")
    print("  - Vision: Score threshold mais baixo (0.4 vs 0.5)")
    print("  - Vision: Learning rate ligeiramente maior")
    print("\n📊 Próximos passos:")
    print("  1. Compare performance com modelo Detector")
    print("  2. Teste em diferentes condições de iluminação")
    print("  3. Calcule métricas (mAP, precision, recall)")
